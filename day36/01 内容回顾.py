...
# 你了解生产者模型消费者模型么？如何实现？
    ...
    # 在python中实现生产者消费者模型可以用哪些机制
        # 消息中间件
        # celery : 定时发短信的任务
# 从你的角度说说进程在计算机中扮演什么角色？  进程是为了更多的占用cpu资源，更少的减少io操作
    # 资源分配的最小单位
    # 进程与进程之间内存隔离
...
# 为什么线程之间数据不安全
    # 线程之间数据共享
    # 多线程的情况下,
        # 如果在计算某一个变量的时候,还要进行赋值（+= -= *= /=）操作,这个过程不是由一条完整的cpu指令完成的
        # 如果在判断某个bool表达式（if while）的之后,再做某些操作,这个过程也不是由一条完整的cpu指令完成的
        # 在中间发生了GIL锁的切换(时间片的轮转),可能会导致数据不安全.

# 同步时线程一定安全，异步时线程可能不安全
...
#     global number
#     for _ in range(loop):
#         while not numbers:    # 当有两个线程是线程会不安全，一个线程pop时安全
#             time.sleep(1E-8)
#         numbers.pop()
# numbers = [0]
...
# 线程锁 - 最好只创建一把锁,线程锁一定锁不了进程
    ...
    # 判断数据是否安全
        # 是否数据共享（全局变量，静态变量等）,是同步还是异步 (数据共享并且异步)
        # += -= *= /= 赋值 = 计算之后 数据不安全
